...
//		System.out.println("product.get::::::"+product.getProTranCode());
		 ...
		 * Cookie("history", prodNo+","+ckHistory); // 쿠키 생성 cookie.setMaxAge(60*60); //
		 * 헌재 Cookie의 유지기간 cookie.setPath("/"); response.addCookie(cookie);
		 * //model.addAttribute("cookie", cookie); // Model 과 View 연결
		 * 
		 * // 쿠키 => 장바구니 -> session
		 */		
